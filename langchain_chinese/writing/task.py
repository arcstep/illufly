...
如果你决定输出“段落内容”，就请按照如下格式输出（你必须考虑正确的语法，以便我能够按照JSON解析）：
```json
{
    "类型": "paragraph",
    "标题名称": 不带编号的标题名称,
...
如果你决定输出“写作提纲”，就请按照如下格式输出（你必须考虑正确的语法，以便我能够按照JSON解析）：
```json
{
    "类型": "outline",
    "标题名称": 不带编号的标题名称,
    ...
    "大纲数量": 与以上列表相符的大纲数量,
    "大纲列表": [
        {"标题名称": 不带编号的标题名称, "总字数要求": 段落的字数要求（int类型）, "扩写指南": 可以包含涉及的人物、地点、情节等实体名称和背景设定},
        ...,
        {"标题名称": 不带编号的标题名称, "总字数要求": 段落的字数要求（int类型）, "扩写指南": 可以包含涉及的人物、地点、情节等实体名称和背景设定}
    ]
}
