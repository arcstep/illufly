...
- 工作台：工作台用于保存对话和写作的上下文环境，包括知识加载、数据处理、操作演示、图表展示等
- 多轮对话：多轮对话支持对话记忆，支持工具回调，支持知识加载，支持写作
- 长文写作：写作基于提示语模板，会清空对话历史，也支持支持工具回调，支持知识加载
- 背景知识：提示语中可插入背景知识，将插入到对话结构中，不受对话长度控制的限制
- 回调工具：数据分析等工具是内置的，也可以支持自定义的工具
- 数据分析：数据分析是内置的，也是重点支持的工具
- FastAPI支持：全框架支持异步、流式输出和SSE
...
4. 支持多智能体：基于Desk和代码逻辑定义
    - [ ] 支持顺序执行
    - [ ] 支持循环执行
    - [ ] 支持并行执行

5. 支持工作台数据持久化：可从历史状态恢复
