...
from illufly.agent.thread import MemoryMessage, HistoryMessage
...
class TestMessage:
    ...
    def message(self):
        """创建一个基本消息"""
        return MemoryMessage(role="user", content="你好")
    ...
    def test_create_from_tuple(self):
        """测试从元组创建消息"""
        # 测试普通角色
        msg1 = MemoryMessage.create(("user", "用户消息"))
        assert msg1.role == "user"
        assert msg1.content == "用户消息"
        
        # 测试 ai 到 assistant 的转换
        msg2 = MemoryMessage.create(("ai", "AI回复"))
        assert msg2.role == "assistant"
        assert msg2.content == "AI回复"
        
        # 测试系统消息
        msg3 = MemoryMessage.create(("system", "系统提示"))
    ...
    def test_create_from_message_list(self):
        ...
        msgs = [MemoryMessage.create(msg) for msg in messages]
    ...
    def test_create_from_message(self):
        """测试从现有消息创建消息"""
        original = MemoryMessage(role="user", content="原始消息")
        copied = MemoryMessage.create(original)
        assert copied.role == original.role
        assert copied.content == original.content
        assert isinstance(copied, MemoryMessage)
        
    def test_message_property(self):
        """测试 message 属性（用于序列化）"""
        msg = MemoryMessage(role="user", content="测试消息")
    ...
    def test_invalid_role(self):
        ...
        with pytest.raises(ValueError):
            MemoryMessage.create(("invalid_role", "消息"))
    ...
    def test_role_validation(self):
        ...
        for role in valid_roles:
            msg = MemoryMessage.create((role, "测试消息"))
        ...
        for role in invalid_roles:
            with pytest.raises(ValueError):
                MemoryMessage.create((role, "测试消息"))