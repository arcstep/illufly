...
class TestTagIndexing:
    ...
    def test_tag_indexing_basic(self, tag_storage_factory):
        ...
        # 验证索引结构
        assert "tags" in storage._indexes
        assert "python" in storage._indexes["tags"]
        assert set(storage._indexes["tags"]["python"]) == {"owner1", "owner2"}
        assert "web" in storage._indexes["tags"]
        assert storage._indexes["tags"]["web"] == ["owner1"]

        # 使用索引进行查询
        with patch.object(storage, '_find_with_index') as mock_index_find:
            mock_index_find.return_value = ["owner1"]
            results = storage.find({"tags": "web"})
            mock_index_find.assert_called_once_with("tags", "web")
            assert len(results) == 1
            assert results[0].id == "1"
    ...
    def test_tag_index_update(self, tag_storage_factory):
        ...
        assert "python" in storage._indexes["tags"]
        assert "web" in storage._indexes["tags"]
        assert storage._indexes["tags"]["python"] == ["owner1"]
        ...
        # 验证索引更新
        assert "web" not in storage._indexes["tags"]
        assert "api" in storage._indexes["tags"]
        assert storage._indexes["tags"]["python"] == ["owner1"]
        assert storage._indexes["tags"]["api"] == ["owner1"]
    ...
    def test_tag_deletion(self, tag_storage_factory):
        ...
        # 验证索引已清理
        assert "owner1" not in storage._indexes["tags"].get("python", [])
        assert "owner1" not in storage._indexes["tags"].get("web", [])
    ...
    def test_tag_index_load(self, tag_storage_factory):
        ...
        # 验证索引是否正确加载
        assert "tags" in storage2._indexes
        assert "python" in storage2._indexes["tags"]
        assert "web" in storage2._indexes["tags"]
        assert storage2._indexes["tags"]["python"] == ["owner1"]
