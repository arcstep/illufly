...
class TestTokenManagement:
    ...
    def test_create_access_token(self, tokens_manager):
        """测试基本的访问令牌创建功能"""
        token_data = {
            "user_id": "test123",
            "username": "testuser",
        ...
        # 创建访问令牌
        access_result = tokens_manager.create_access_token(token_data)
        ...
        # 验证访问令牌
        verify_result = tokens_manager.verify_jwt(access_token)
    ...
    def test_create_refresh_token(self, tokens_manager, test_user):
        ...
        result = tokens_manager.create_refresh_token(token_data)
    ...
    def test_token_expiration(self, tokens_manager, test_user):
        """测试令牌过期"""
        user = test_user
        token_data = {
        ...
        result = tokens_manager.create_access_token(token_data)
        assert result["success"] is True
        verify_result = tokens_manager.is_token_valid(result["token"], "access")
        assert verify_result["success"] is False
        assert "expired" in verify_result["error"].lower()

    def test_invalidate_access_token(self, tokens_manager):
        """测试撤销访问令牌"""
        print("\n=== 测试撤销访问令牌 ===")
        ...
        # 验证初始状态
        verify_result = tokens_manager.is_token_valid(token, "access")
        ...
        # 撤销令牌
        invalidate_result = tokens_manager.invalidate_access_token(token)
    ...
    def test_invalidate_refresh_token(self, tokens_manager, test_user):
        """测试撤销刷新令牌"""
        # 1. 创建一个刷新令牌
        token_data = {
            "user_id": test_user.user_id,
        ...
        # 解码令牌以获取过期时间
        decoded = tokens_manager.verify_jwt(refresh_token)
        ...
        # 存储令牌
        tokens_manager._storage.set(value=storage, owner_id=test_user.user_id)
        
        # 2. 验证令牌初始有效
        stored_token = tokens_manager._storage.get(owner_id=test_user.user_id)
        ...
        verify_result = tokens_manager.is_token_valid(refresh_token, "refresh")
        print(f"Initial verify result: {verify_result}")
        assert verify_result["success"] is True, f"Token should be valid initially: {verify_result.get('error')}"
        
        # 3. 撤销令牌
        invalidate_result = tokens_manager.invalidate_refresh_token(refresh_token)
        print(f"Invalidate result: {invalidate_result}")
    ...
    def test_invalidate_user_access_tokens(self, tokens_manager, test_user):
        """测试撤销用户所有访问令牌"""
        # 创建多个访问令牌
        tokens = []
        ...
        # 使用 user_id 撤销令牌
        result = tokens_manager.invalidate_user_access_tokens(test_user.user_id)
        assert result["success"] is True
        assert f"Removed {len(tokens)} access tokens" in result["message"]

    def test_invalidate_user_refresh_tokens(self, tokens_manager, test_user):
        """测试撤销用户所有刷新令牌"""
        tokens = []
        for device in ["mobile", "desktop", "tablet"]:
            token_data = {
        ...
        result = tokens_manager.invalidate_user_refresh_tokens(test_user.user_id)
        assert result["success"] is True

    def test_invalidate_all_user_tokens(self, tokens_manager, test_user):
        """测试撤销用户所有令牌"""
        access_tokens = []
        refresh_tokens = []
        storage = TokenStorage()
        ...
        for device in ["mobile", "desktop"]:
            ...
            # 创建刷新令牌
            refresh_result = tokens_manager.create_refresh_token(token_data)
        ...
        # 2. 撤销所有令牌
        result = tokens_manager.invalidate_all_user_tokens(test_user.user_id)
        assert result["success"] is True
        tokens_count = len(refresh_tokens)
        expected_message = f"Removed {tokens_count} access tokens, {tokens_count} refresh tokens"
        assert result["message"] == expected_message

    def test_invalidate_nonexistent_tokens(self, tokens_manager):
        """测试撤销不存在的令牌"""
        invalid_token = "invalid.token.string"
        
        # 测试撤销不存在的访问令牌
        result = tokens_manager.invalidate_access_token(invalid_token)
        assert result["success"] is False
        assert "Invalid token" in result["error"]
        
        # 测试撤销不存在的刷新令牌
        result = tokens_manager.invalidate_refresh_token(invalid_token)
        assert result["success"] is False
        assert "Invalid token" in result["error"]
