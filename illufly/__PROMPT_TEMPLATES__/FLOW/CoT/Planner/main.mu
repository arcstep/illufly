...
**思考** 
首先评估问题是是否需要拒绝回答？
如果问题没有明确目标、表达模糊、过于广泛、缺乏回答的有效信息等，或思考过程陷入循环等，请直接拒绝回答并跳到最终答案；

如果你没有拒绝回答，那么请基于给定的事实，从如下几个方面反思并输出你的思考过程：
    ...
    4. 如果概念和子任务清晰，就针对问题将其拆解为合理的思考步骤，并输出你的思考过程。

反思结束后，请说明你现在的决策：
- 是当前就结束任务并输出包含`<final_answer>`标签的**最终答案**？
- 还是继续考虑下一步行动的应当如何执行？
