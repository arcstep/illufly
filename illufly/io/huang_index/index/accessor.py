...
class SequenceAccessor(ValueAccessor):
    ...
    def can_handle(self, value: Any) -> bool:
        """检查是否可以处理序列类型"""
        self._logger.info(f"SequenceAccessor.can_handle: value={value}, type={type(value)}")
        
        # 处理泛型类型
        if hasattr(value, '__origin__'):
            origin = value.__origin__
            self._logger.info(f"泛型类型: origin={origin}")
            return origin in (list, List)
        
        # 处理普通类型
        if isinstance(value, type):
            self._logger.info(f"普通类型: {value}")
            return issubclass(value, (list, List)) and not issubclass(value, (str, bytes))
        
        # 处理实例
        self._logger.info(f"实例类型: {type(value)}")
...
class MappingAccessor(ValueAccessor):
    ...
    def can_handle(self, value: Any) -> bool:
        """检查是否可以处理映射类型"""
        self._logger.info(f"MappingAccessor.can_handle: value={value}, type={type(value)}")
...
class CompositeAccessor(ValueAccessor):
    ...
    def __init__(self, logger: Optional[logging.Logger] = None):
        ...
        self._logger.info(f"初始化访问器列表: {[type(acc).__name__ for acc in self._accessors]}")
    ...
    def validate_path(self, value_type: Type, path_segments: Tuple[PathSegment, ...]) -> Optional[str]:
        ...
        self._logger.info(f"开始验证路径: value_type={value_type}, segments={path_segments}")
        if hasattr(value_type, '__origin__'):
            self._logger.info(f"类型详情: origin={value_type.__origin__}, args={getattr(value_type, '__args__', None)}")
            
        # 找到合适的访问器
        accessor = None
        for acc in self._accessors:
            self._logger.info(f"尝试访问器: {type(acc).__name__}")
            if acc.can_handle(value_type):
                accessor = acc
                self._logger.info(f"找到合适的访问器: {type(acc).__name__}")
                break
            else:
                self._logger.info(f"访问器 {type(acc).__name__} 无法处理类型 {value_type}")
        ...
        # 如果有更多路径段，递归验证
        if len(path_segments) > 1:
            self._logger.info(f"继续验证剩余路径段: {path_segments[1:]}")
            ...
            try:
                ...
                self._logger.info(f"获取到下一级类型: {next_type}")
    ...
    def can_handle(self, value: Any) -> bool:
        """检查是否可以处理特定值或类型"""
        self._logger.info(f"CompositeAccessor.can_handle: value={value}, type={type(value)}")
        
        for accessor in self._accessors:
            self._logger.info(f"尝试访问器: {type(accessor).__name__}")
            can_handle = accessor.can_handle(value)
            self._logger.info(f"访问器 {type(accessor).__name__} can_handle={can_handle}")
            
            if can_handle:
                self._logger.info(f"找到合适的访问器: {type(accessor).__name__}")
                return True
