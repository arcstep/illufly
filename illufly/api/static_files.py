...
class StaticFilesManager:
    """静态文件管理器"""
    def __init__(self, package="illufly", static_dir="api/static", logger=None):
    ...
    def setup(self) -> Path:
        ...
        try:
            ...
            with importlib.resources.files(self.package).joinpath(self.static_dir) as static_path:
                self.logger.debug(f"拷贝资源目录: {static_path}")
                for file_path in static_path.rglob("*"):
                    if file_path.is_file():
                        ...
                        self.logger.debug(f"拷贝文件: {dest_path}")
            
            self.logger.debug(f"静态文件已复制到临时目录: {temp_static}")
            return temp_static
            
        except Exception as e:
            self.logger.warning(f"静态文件设置失败: {e}")
            return None
    ...
    def cleanup(self):
        ...
        if self._temp_dir and Path(self._temp_dir).exists():
            ...
            self.logger.warning(f"已清理临时目录: {self._temp_dir}")
