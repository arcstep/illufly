...
def create_auth_endpoints(app, db: IndexedRocksDB, prefix: str="/api"):
    ...
    def set_auth_cookies(response: Response, access_token: str = None) -> None:
        """设置认证Cookie"""
        try:
            if access_token:
                response.set_cookie(
    ...
    async def verified_user(
        ...
        if token_claims.iat + timedelta(seconds=1) > datetime.now():
            tokens_manager.set_auth_cookies(
                response,
                access_token=token_claims.jwt_token()
            )
